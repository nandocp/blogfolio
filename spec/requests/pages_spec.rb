...
RSpec.describe "Pages", type: :request do
  ...
  describe "GET /portfolio" do
    before(:all) do
      create_list(:portfolio_entry, 15)
      get portfolio_path
    end
    ...
    it 'Should be processed by the correct action' do
      expect(@controller.action_name).to eq('portfolio')
    end

    it 'Should return the correct view' do
      expect(response.body).to include("id='portfolio-posts'")
    end

    it 'Should return the correct number of entries' do
      _test = @controller.instance_variable_get(:@portfolio_entries)
      expect(_test.length).to eq(15)
      expect(_test.all? { |entry| entry.is_a?(Portfolio) }).to be_truthy
    end
  end
end
